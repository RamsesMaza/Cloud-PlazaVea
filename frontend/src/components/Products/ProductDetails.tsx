...
const ProductDetails: React.FC<ProductDetailsProps> = ({ productId, onClose, onEdit }) => {
  ...
  // Asegurar que price siempre sea number
  ...
  const getStockStatus = () => {
    if (product.stock === 0)
      return { color: 'text-red-600', label: 'Agotado', bgColor: 'bg-red-50' };

    if (product.stock <= product.minStock)
      return { color: 'text-orange-600', label: 'Stock Bajo', bgColor: 'bg-orange-50' };

    if (product.stock >= product.maxStock)
      return { color: 'text-blue-600', label: 'Stock Alto', bgColor: 'bg-blue-50' };
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="text-2xl font-bold">
                  S/ {(product.stock * price).toFixed(2)}
                </div>
            ...
            {product.stock <= product.minStock && (
              <div className="mt-4 p-4 bg-orange-50 border border-orange-200 rounded-lg flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-orange-600 text-sm">Se recomienda reponer producto</p>
                </div>
              </div>
            )}
          </div>
